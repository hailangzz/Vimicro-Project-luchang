...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        if opt.ft_pruned_model:
            srtmp = 0
            model = Model(ckpt["models"].yaml, ch=3, nc=nc, anchors=hyp.get(
                'anchors'), mask_bn=ckpt["models"].mask_bn).to(device)  # create
        else:
            model = Model(cfg or ckpt['models'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
...
def parse_opt(known=False):
    ...
    parser.add_argument('--ft_pruned_model', action='store_true', help='finetune pruned models')
...
def main(opt, callbacks=Callbacks()):
    ...
    if LOCAL_RANK != -1:
        ...
        assert not opt.image_weights, '--images-weights argument is not compatible with DDP training'
