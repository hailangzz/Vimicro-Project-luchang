...
"""
Train a YOLOv5 segment models on a segment dataset
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        model = SegmentationModel(cfg or ckpt['models'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _, masks) in pbar:  # batch ------------------------------------------------------
            ...
            if RANK in {-1, 0}:
                ...
                # callbacks.run('on_train_batch_end', models, ni, imgs, targets, paths)
            ...
            if f.exists():
                ...
                if f is best:
                    ...
                    results, _, _ = validate.run(
                        ...
                        compute_loss=compute_loss,
                        mask_downsample_ratio=mask_ratio,
                        overlap=overlap)  # val best models with plots
                    if is_coco:
