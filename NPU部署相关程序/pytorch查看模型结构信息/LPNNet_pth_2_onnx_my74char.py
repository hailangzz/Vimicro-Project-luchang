...
class LPRNet(nn.Module):
    def __init__(self, lpr_max_len, phase, class_num, dropout_rate):
        ...
        self.container = nn.Sequential(
            nn.Conv2d(in_channels=224+self.class_num, out_channels=self.class_num, kernel_size=(1, 1), stride=(1, 1)),
            nn.AvgPool3d(kernel_size=(1, 4, 1))
        ...
        self.conv_p0 = nn.Sequential(
            nn.Conv2d(in_channels=64, out_channels=32, kernel_size=(3, 3), stride=5),
            nn.BatchNorm2d(num_features=32),
            nn.ReLU(),
        )
        self.conv_p1 = nn.Sequential(
            nn.Conv2d(in_channels=128, out_channels=64, kernel_size=(3, 3), stride=5),
            nn.BatchNorm2d(num_features=64),
            nn.ReLU(),
        )
        self.conv_p2 = nn.Sequential(
            nn.Conv2d(in_channels=256, out_channels=128, kernel_size=(3, 3), stride=(3,2)),
            nn.BatchNorm2d(num_features=128),
            nn.ReLU(),
            nn.MaxPool3d(kernel_size=(1, 3, 4), stride=(1, 1, 1)),
        )
    ...
    def forward(self, x):
        ...
        for i, f in enumerate(keep_features):
            ...
            #             #     f = nn.Conv2d(in_channels=256, out_channels=class_num, kernel_size=(13, 1), stride=1)(f)
            #             # if i in [2]:
            ...
            if i==0:
                ...
                f = self.conv_p0(f)
#                f = nn.Conv2d(in_channels=64, out_channels=32, kernel_size=(3, 3), stride=5)(f)
#                f = nn.BatchNorm2d(num_features=32)(f)  # 1  -> [bs,64,22,92]
#                f = nn.ReLU()(f)
            if i==1:
                f = self.conv_p1(f)
#                f = nn.Conv2d(in_channels=128, out_channels=64, kernel_size=(3, 3), stride=5)(f)
#                f = nn.BatchNorm2d(num_features=64)(f)  # 1  -> [bs,64,22,92]
#                f = nn.ReLU()(f)
            if i==2:
                f = self.conv_p2(f)
#                f = nn.Conv2d(in_channels=256, out_channels=128, kernel_size=(3, 3), stride=(3,2))(f)
#                f = nn.BatchNorm2d(num_features=128)(f)  # 1  -> [bs,64,22,92]
#                f = nn.ReLU()(f)
#                f = nn.MaxPool3d(kernel_size=(1, 3, 4), stride=(1, 1, 1))(f)
        ...
        logits = torch.squeeze(x, dim=2)
        return logits




model = LPRNet(lpr_max_len=8,phase=True,class_num=74,dropout_rate=1)
# device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
device = torch.device('cpu')
model.load_state_dict(torch.load(r'D:\中星微人工智能工作\Total_Models\yolov5-LPRNet-车牌检测识别\runsLPRNet__iteration_30.pth', map_location=device))
model.eval()

input_names = ['input']
output_names = ['output']

x = torch.randn(1, 3, 24, 94, device=device)

torch.onnx.export(model, x, 'LPNNet_my74.onnx', input_names=input_names, output_names=output_names)