...
class cut_image_from_vidoe():
    ...
    def __init__(self):
        # self.origin_vidoe_path = r"F:\AiTotalDatabase\ADAS_实车测试录像"
        # self.total_vidoe_path_list = os.listdir(self.origin_vidoe_path)
        # self.save_path = r'F:\AiTotalDatabase\ADAS_origin_images'

        self.origin_vidoe_path = r"F:\AiTotalDatabase\adas_demo\video"
        self.total_vidoe_path_list = os.listdir(self.origin_vidoe_path)
        self.save_path = r'F:\AiTotalDatabase\adas_demo\images'
    ...
    def getFrame(self,frame_interval):
        ...
        for vidoe_index in tqdm(range(len(self.total_vidoe_path_list))):
            ...
            print(fps)
            frame_interval = int(fps/25) # 根据视频帧计算每秒取一针
            # print(fps) # 30，为每秒30帧:
            while True:
                if cap.grab(): # 用来指向下一帧
                    '''
