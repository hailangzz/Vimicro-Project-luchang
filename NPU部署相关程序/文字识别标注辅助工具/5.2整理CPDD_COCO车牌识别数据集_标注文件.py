...
total_rec_path_info={
                     'train':r'D:\迅雷下载\AI数据集汇总\汽车拍照检测识别\detect_car_brand_crops\all_detect_crops_database_labels_file\detect_CPDD_coco_car_brand_focus_8_pt_labels',
...
def create_rec_image_dataset(origin_image_labels_path,save_CPDD_labels_path):

    save_labels_cur = open(save_CPDD_labels_path,'w',encoding='utf-8')

    for filename in os.listdir(origin_image_labels_path):
        real_label_str =''
        ...
        save_labels_cur.write(filename.replace('.txt','_0.jpg')+'\t'+real_label_str+'\n')
...
save_CPDD_labels_path=r'D:\迅雷下载\AI数据集汇总\汽车拍照检测识别\detect_car_brand_crops\all_detect_crops_database_labels_file\detect_CPDD_coco_car_brand_crops_focus_8_pt_labels.txt'
create_rec_image_dataset(total_rec_path_info['train'],save_CPDD_labels_path)
