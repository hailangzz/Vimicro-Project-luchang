...
def get_voc_data(train_and_valid_list,voc_data_dir,save_yolov3_coco_data_dir):
    for use_type in train_and_valid_list: # 'train.txt','valid.txt'
        use_type_info_dict={'images_path_list':[],'annotations_path_list':[],'images_name_list':[]}

        second_dir = os.path.join(save_yolov3_coco_data_dir,use_type.split('.')[0]) #次级存储txt标注文件的存储路径

        if not os.path.exists(second_dir):
            os.mkdir(second_dir)
        use_type_list_info = open(os.path.join(voc_data_dir,use_type),'r').readlines() #读取'train.txt','valid.txt' 中images对应的标注文件记录信息

        for single_image_info in use_type_list_info:
            single_image_info_list = single_image_info.strip().replace('./',"").split(' ') #./images/road839.png ./annotations/road839.xml
            single_image_info_list[0]=os.path.join(voc_data_dir,single_image_info_list[0]) #./images/road839.png
            single_image_info_list[1] = os.path.join(voc_data_dir, single_image_info_list[1]) #./annotations/road839.xml

            use_type_info_dict['images_path_list'].append(single_image_info_list[0]) #图片样本路径列表
            use_type_info_dict['annotations_path_list'].append(single_image_info_list[1])#xml标注路径列表
            use_type_info_dict['images_name_list'].append(single_image_info_list[1].split('/')[-1].split('.')[0])#图像样本名称列表road839
