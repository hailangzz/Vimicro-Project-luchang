...
def cut_crops_function(image_path, box_info_list,save_detect_crops_path,train_image_size=[1024,576]):
    ...
    for detect_box_info_index in range(len(box_info_list)):
        ...
        x1, y1, x2, y2 = xywh2xyxy(detect_box_info)
...
images_path = r'D:\迅雷下载\AI数据集汇总\汽车拍照检测识别\CCPD_COCO_dataset\train\images'
detect_labels_path =r'D:\迅雷下载\AI数据集汇总\汽车拍照检测识别\detect_car_brand_crops\all_detect_crops_database_labels_file\detect_CPDD_coco_car_brand_focus_8_pt_labels'

save_detect_crops_path = r'D:\迅雷下载\AI数据集汇总\汽车拍照检测识别\detect_car_brand_crops\detect_CPDD_coco_car_brand_crops_focus_8_pt' # 保存检测目标剪切图片的路径
