...
@torch.no_grad()
def run(
        weights=ROOT / 'yolov5s.pt',  # models.pt path(s)
    ...
    # Print results
    t = tuple(x / seen * 1E3 for x in dt)  # speeds per images
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='result_300epoch/best.pt', help='models path(s)')      # 训练好的权重
    parser.add_argument('--source', type=str, default='inference/images', help='file/dir/URL/glob, 0 for webcam')       # 待测试数据
    parser.add_argument('--data', type=str, default='dota_data/dota_name.yaml', help='(optional) dataset.yaml path')    # 数据集文件
    ...
    parser.add_argument('--project', default='inference/output', help='save results to project/name')                   # 测试结果保存路径
    parser.add_argument('--name', default='exp', help='save results to project/name')                                   # 测试结果保存的子目录
