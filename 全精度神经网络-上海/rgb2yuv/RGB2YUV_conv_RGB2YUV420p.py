...
class Net(nn.Module):
   ...
   def forward(self, img):
      ...
      # print(out.size())
      Y = self.conv_420Y(out).type(torch.uint8)/255.0#-0.429 )/pow(0.253,0.5)
      UV = self.conv_420UV(out).type(torch.int8)#-torch.tensor([0.001265,-0.005162]).reshape(2,1,1).to(device))/torch.tensor([pow(0.006628,0.5),pow(0.006625,0.5)]).reshape(2,1,1).to(device)
...
if __name__=='__main__':
   ...
   save_model(net, 'RGB2YUV420p_1_new.pth')
   ...
   print("**********pred_UV************")
   print(pred_UV)
   Y = (pred_Y*255).astype(np.uint8)
   UV = (pred_UV).astype(np.int8)
   print("**********Y************")
   print(Y)
   print("**********UV************")
   print(UV)
   yuv_file = 'test_yuv420p_1_new.yuv'
