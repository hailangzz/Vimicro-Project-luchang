...
class create_new_mask_sample:
    ...
    def create_random_shape_mask_img(self,resize=(24,24)):
        ...
        mask_img = self.transPNG(choice_mask_info['image_path'])
...
class Create_train_sample:
    def __init__(self):
        ...
        self.save_coco_dataset_path = r"D:\迅雷下载\AI数据集汇总\红绿灯检测数据集\traffic_light_detection\traffic_light_detection\traffic_light_detection_coco_buff2"
            ...
            for mask_classify in train_label_info['box_info']:
                for single_box in train_label_info['box_info'][mask_classify]:
                    try:
                        ...
                        new_resize_mask = new_resize_mask.filter(
                            ImageFilter.GaussianBlur(radius=random.uniform(0.1, 0.5)))  # 图像增加高斯模糊···

                        layer = Image.new('RGBA', origin_image.size, (0, 0, 0, 0))

                        # print(single_box)
                        single_box[0] = int(single_box[0]+wight*0.09)
                        single_box[1] = int(single_box[1]+hight*0.09)
                        single_box[2] = int(wight*0.9)+single_box[0]
                        single_box[3] = int(hight*0.9)+single_box[1]

                        # print(single_box)
                        layer.paste(new_resize_mask, (single_box[0],single_box[1]))
                        origin_image = Image.composite(layer, origin_image, layer)
    ...
    def read_sample_info(self,create_new_sample=False,my_create_sample_number=42000):
        print('hello word!')
        ...
        if not create_new_mask_sample: # 当不是自定义构造样本时
            ...
            sample_num = 0
            my_create_sample_number = my_create_sample_number
            for lable_xml_index in tqdm(range(my_create_sample_number)):
                ...
                if sample_num < my_create_sample_number*0.95:
...
a.read_sample_info(create_new_sample,my_create_sample_number=50000)
