...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    ...
    if bpr > 0.98:  # threshold to recompute
        print(f'Current anchors are a good fit to dataset ✅')

    else:
        print(f'Anchors are a poor fit to dataset ⚠️, attempting to improve...')
...
def kmean_anchors(path='./data/ccpd.yaml',anchor_data_type='train', n=9, img_size=640, thr=4.0, gen=1000, verbose=True):
    ...
    if isinstance(path, str):  # *.yaml file
        ...
        dataset = LoadImagesAndLabels(data_dict[anchor_data_type], augment=True, rect=True)
    else:
...
kmean_anchors(path=r'./ccpd_new.yaml',anchor_data_type = 'train',img_size=(736,1280),thr=2)