...
class Factory_Compare_Image_Info(): # 批量化比较图片相似度排序信息
    ...
    def get_images_simcos(self,image_similar_boject,origin_image_path,stand_image_name=''):
        ...
        for i in tqdm(range(len(total_images_name))):
            ...
            if number==0 and stand_image_name=='':
                stand_image_path = os.path.join(origin_image_path, single_image_name)
            else:
                stand_image_path = os.path.join(origin_image_path, stand_image_name)
...
stand_image_name = 'c.jpeg'
factory_compare_image.get_images_simcos(imag_similar,origin_image_path,stand_image_name)
